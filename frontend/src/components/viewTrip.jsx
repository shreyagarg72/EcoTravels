...
const ViewTrip = () => {
  ...
  const auth = getAuth();
  const navigate = useNavigate(); // For navigation to flight details page
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      ...
      if (docSnap.exists()) {
        ...
        // Store trip data in localStorage for access in the FlightDetails component
        localStorage.setItem('tripData', JSON.stringify(tripData));
        
        if (tripData?.tripData?.travelPlans) {
  ...
  const handleViewFlights = () => {
    navigate('/flight-details', { state: { trip } });
  };
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-md flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white transition-colors"
          >
            <svg
