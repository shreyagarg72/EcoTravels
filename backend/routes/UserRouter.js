...
router.post("/users/:firebaseUid/updateTier", async (req, res) => {
  ...
  try {
    ...
    user.ecoPoints.history.push({
      ...
      description:
        description || `Subscription changed from ${previousTier} to ${tier}`,
      timestamp: new Date(),
    });
...
// Save a trip ID to user's saved trips array
router.post("/save-trip", async (req, res) => {
  try {
    ...
    // Initialize savedTrips array if it doesn't exist
    if (!user.savedTrips) {
      user.savedTrips = [];
    }
    ...
    // Add trip ID to saved trips array
    user.savedTrips.push(tripId);

    // Add eco points for saving a trip
    const ecoPointsToAdd = 10;
    user.ecoPoints.total += ecoPointsToAdd;

    // Add to points history
    user.ecoPoints.history.push({
      points: ecoPointsToAdd,
      activityType: "save_trip",
      description: "Saved a trip",
      timestamp: new Date(),
    });

    // Update user tier based on new points total if needed
    if (user.ecoPoints.total >= 1000) {
      user.ecoPoints.tier = "Premium";
    } else if (user.ecoPoints.total >= 500) {
      user.ecoPoints.tier = "Standard";
    }
